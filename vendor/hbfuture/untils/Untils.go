...
// return: 请求结果
func HttpPostRequest(strUrl string, mapParams map[string]string) (string,*http.Response,error) {
	httpClient := &http.Client{}
...
// strRequest: API路由路径
// return: 请求结果
func ApiKeyPost(mapParams map[string]string, strRequestPath string) (string,*http.Response,error) {
	strMethod := "POST"
	timestamp := time.Now().UTC().Format("2006-01-02T15:04:05")
	// t := time.Now()
	...
	jsonStr, response, err := HttpPostRequest(strUrl, mapParams)
	return jsonStr,response,err
}
